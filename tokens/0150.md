...
The same proof material MAY be delivered outside basket JSON as binary [Outpoint BEEF](../transactions/0158.md) (BRC-158) for the tip; this BRC’s `provenance` object remains the basket remittance shape.
...
7. **Ordinal continuity** — For each consecutive pair `(child, parent)` in `path` with different transaction ids, the child transaction in the BEEF MUST spend `parent` as an input, and calculating the ordinal on the child (using the included input source transactions) MUST show the parent sat continues to the child path outpoint. For parent-remittance acceptance (step 5), the spend `T → P` is checked against transaction `T` (which MAY be obtained outside `beefB64`).
   ...
   Informative procedure for calculating the ordinal at a hop (and walking tip→origin when desired):
...
* The tip→origin proof MAY also be transported as binary [Outpoint BEEF](../transactions/0158.md) ([BRC-158](../transactions/0158.md)); basket remittance remains this JSON `provenance` object.
