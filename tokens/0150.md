...
* If `beefB64` supports calculating the ordinal from `tip` through to the first `ord` envelope, is `path` still required, or could it become optional?
* Are `origin` and `contentType` still useful once verification recovers origin (and type) from the envelope, or only as UX shortcuts?
* Given that the proven object is an outpoint (`txid_vout`), does full BEEF ([BRC-62](../transactions/0062.md)) fit remittance better than AtomicBEEF ([BRC-95](../transactions/0095.md)), whose subject is a transaction id?
