# BRC-228: Multicast Shard Domain Partitioning and the BEEF Object Plane
...
frames, and manifests are byte-identical to their pre-BRC-228 form; this is a
...
(`TopicCount > 1`) is an authenticated-ingress capability (BRC-229 §Fan-out
...
specified in [BRC-229](./0229.md), which assigns `FrameVersion 0x09`. This
...
- **ContentID (offset 8, 32 bytes)** — SHA-256d over the complete object
  ...
  **paired with TopicID** — sibling emissions of one object to different
  topics share a ContentID, so a ContentID-only slot key would collapse them
  and deliver only one — hence two in-flight objects for one subject must
  never share it.
...
Domains section and remains correct for the transaction plane. A BRC-228
...
- [BRC-229: Multicast BEEF Object Frame Format](./0229.md)
