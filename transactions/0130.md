...
frames `0x01`/`0x02`, and for BRC-229 BEEF object frames `0x00` (their byte 7 is
...
`0x04` BRC-131, `0x05` BRC-132, `0x09` BRC-229 BEEF object). A value of `0x00`
defaults to
...
1. **Slot allocation.** On receipt of the first fragment for a reassembly key,
   ...
   frames legitimately share. For `OrigFrameVer = 0x09` (BRC-229 BEEF object)
...
4. **Hash verification.** Where bytes 8–39 are a double-SHA256 of the payload —
   `OrigFrameVer` `0x00`/`0x02` (transaction, TxID) and `0x09` (BRC-229 BEEF
...
5. **Delivery.** Construct a synthetic frame of the original type — `FrameVer =
   ...
   object (`0x09`) is therefore rebuilt as a BRC-229 frame with its ContentID
