...
provisional (the next unassigned frame-version code). The header reuses the
...
| 8 | 32 | `[32]byte` | ContentID | `SHA-256d(payload bytes)` — the object's identity; keys ingress duplicate suppression and BRC-130 fragment reassembly. Never the subject TxID. |
