...
A gate asks what a handle holds, which is a different question from which key signs for it. Assets sit at derived or separate keys, so evaluating a holding gate already requires a mapping from a handle to the outputs it controls, and that mapping is not the identity binding. It follows that rotating an identity key does not move anybody's coins: where the ecosystem's resolution carries the new binding, the holdings that were there are still there, and a client that zeroes a reader's holdings because their identity key changed has confused two different lookups.
...
Three further types were designed and are left unspecified, recorded so the next author does not rediscover them or ship them incompatibly.
