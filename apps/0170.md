...
8. **Every payee receives payment evidence directly.** Multilateral payment uses one transaction with multiple outputs, but the Atomic BEEF and remittance for each output are submitted to the corresponding payee system.
...
- The proposed [BRC-167 CHIRP](https://github.com/bsv-blockchain/BRCs/pull/235), when adopted, defines a chunked Merkle-object layer above UHRP. LCH does not change its integrity, resolution, or hosting rules, and does not normatively depend on the unmerged proposal.
...
`asset`, `header`, `authority`, `offer`, `selection`, `license-request`, `quote`, `payment-demand`, `payment-delivery`, `payment-receipt`, `license`, or `composition-record`.
...
offer-body = {
  ...
  "keyDelivery": { * tstr => any },
  "enforcement": { * tstr => any },
  "notBefore": uint,
  ? "notAfter": uint,
  "nonce": bstr .size 16,
  ? "extensions": { * tstr => any },
...
The ODRL Offer `uid` MUST equal the LCH Offer IRI computed from the Offer Body. Because this creates a circular dependency if the final ID is inserted before hashing, the transmitted JSON-LD MUST instead use the literal placeholder IRI `lch:offer:self`. During ODRL processing only, it is interpreted as the computed Offer IRI. The literal bytes remain unchanged for digest and evidence purposes.
...
The Agreement Policy MUST target the Asset, identify the subject as assignee, include the selected action and Selection, preserve relevant Prohibitions and downstream Duties, and bind all terms accepted by the buyer. Each fulfilled compensation Duty maps to one or more signed Payment Receipt IDs.

The License Agreement `uid` uses the literal `lch:license:self` in transmitted Policy bytes and is interpreted during ODRL processing as the computed License IRI, using the same self-reference rule as Offers.
...
Each Demand MUST be signed by its Payee and use a fresh, single-use `derivationPrefix` generated or approved by that Payee system. The Quote MUST be signed by the License issuer. The Quote total MUST equal the sum of Demand amounts and the ODRL compensation duties.

Before creating a transaction, the client MUST contact every Demand endpoint directly and verify that it recognizes the unexpired Demand and is ready to accept delivery. A coordinator's assertion of reachability is insufficient.
...
3. generate a fresh unique 32-byte `derivationSuffix` for the output;
4. use the Demand Payee identity as counterparty;
5. pay exactly the Demand `satoshis` amount; and
6. set BRC-100 `randomizeOutputs` to `false` so output indices remain stable.
...
All required outputs MUST be in the same subject transaction. Extra unrelated outputs are allowed, but an LCH License issuer and Payees inspect only the Demand-bound outputs. The transaction is transported as one Atomic BEEF.
...
The Payee MUST verify the Demand, identity, amount, derived locking script, output index, Atomic BEEF, transaction validity under its wallet policy, and single-use prefix before calling `internalizeAction` with the appropriate payment remittance.
...
The buyer submits all Receipts and the Atomic BEEF to the License issuer. The issuer MUST independently verify every required output and Receipt before issuing the License. Thus a Payee need not scan the chain, and a coordinator cannot omit delivery while claiming that an output probably exists.

### 11.4 Failure and recovery

One transaction makes the outputs atomic on chain, but direct HTTP deliveries can fail independently. Preflight reduces but cannot eliminate that risk. Payee systems MUST accept recovery delivery for a transaction created before Demand expiry for at least the Offer's declared recovery period. The client MUST retain the Quote, Atomic BEEF, remittance, and successful Receipts until it obtains the License.
...
A License MUST NOT contain a CEK for an unlicensed key period. Possession of ciphertext or a CHIRP leaf does not imply entitlement to every encrypted segment inside it.
...
An LCH Asset created through `odrl:aggregate`, `odrl:extract`, `odrl:derive`, or LCH `train` MUST carry a C2PA 2.4 Manifest reference in `asset.provenance` and a Composition Record in `asset.composition`.
...
Every source License MUST grant the action used to create the derivative and cover `sourceSelection`. `c2paIngredient` MUST reproduce the C2PA hashed URI that resolves to the corresponding ingredient assertion in the referenced C2PA Manifest; `alg` is omitted only where C2PA unambiguously inherits it from the Claim. `c2paManifestDigest` MUST equal `asset.provenance.digest`.
...
2. map the intersection to the applicable `sourceSelection` under the composition profile;
...
- Requires a C2PA `inputTo` ingredient when the training process produces an Asset for which this BRC requires a Composition Record.
...
5. **DAW composition.** A loop License grants `derive` for a selected interval and requires a downstream Policy. The exported song contains a C2PA v3 `componentOf` ingredient, `c2pa.placed` and `c2pa.mixed` actions, an LCH Composition Record, and the required `nextPolicy`.
...
7. **AI training.** A dataset Offer grants `train` for a stated purpose but not `display`, `unwrap`, or `distribute`. A conforming trainer records `inputTo` provenance where required and rejects an attempted raw export.
8. **Rental.** A License permits `play` until a fixed time and allows 24 hours offline. Playback works offline inside that interval and fails after it. A later status change does not claim to erase a raw CEK previously delivered.
9. **Unknown semantics.** A client that understands the Header but not a critical policy term or mechanism rejects that acquisition profile and may select another fully supported profile.
10. **Tampering.** Modified Header CBOR, wrong ciphertext digest, wrong CEK, reordered records, modified AAD fields, reused payment demands, missing Payee receipts, C2PA ingredient mismatch, authority cycles, and composition cycles are all rejected.
...
- BRC-29-derived multi-payee locking scripts, output indices, Demand IDs, and Receipt IDs;
- C2PA ingredient-to-Composition-Record bindings;
- nested-obligation flattening and cycle rejection; and
...
| `ERR_LCH_DELIVERY` | A Payee did not acknowledge direct transaction delivery |
...
Multilateral transactions expose that the outputs were paid together. Direct delivery exposes the buyer and request to every Payee under the authenticated profile. Privacy-enhancing future profiles must state what linkages they remove and must not weaken payment verification silently.
...
This BRC is not legal advice and does not determine copyright ownership, contract formation, consumer rights, exhaustion, fair use or fair dealing, privacy obligations, choice of law, tax, securities treatment, or the applicability of anti-circumvention law. A cryptographic claim of authority can be false in the real world. Human-readable terms and applicable law remain controlling where machine policy is incomplete or legally ineffective.
...
8. Authority Grants;
9. multilateral direct settlement; and
10. C2PA composition and nested downstream policies.
