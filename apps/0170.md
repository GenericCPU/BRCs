...
The client MUST persist the Quote, finalized Atomic BEEF, every remittance, and every signed Payment Delivery before the first network delivery. It then fans the Deliveries out independently, retains each valid Receipt as it arrives, and submits completion only after it has exactly one valid Receipt for every Demand. A failed or slow Payee delivery MUST NOT cause already successful Payee deliveries to be discarded or the transaction to be recreated. The License issuer coordinates completion but is not an implicit payment relay and MUST NOT substitute its own acknowledgement for a Payee Receipt.
...
The buyer submits all Receipts and the Atomic BEEF to the License issuer. Before issuing the License, the issuer MUST independently verify the transaction ID, every required and distinct output index, exact amounts, Demand and Request bindings, Payee signatures, and Receipt IDs. The receiving Payee's signed Receipt attests the BRC-29 locking-script and wallet-policy checks that require its private derivation context; an issuer that is not that Payee cannot reproduce that private derivation. Thus a Payee need not scan the chain, and a coordinator cannot omit delivery while claiming that an output probably exists.

### 11.4 Failure and recovery

One transaction makes the outputs atomic on chain, but direct HTTP deliveries can fail independently. Preflight reduces but cannot eliminate that risk. Payee systems and the License issuer MUST accept idempotent recovery while the current time is less than the signed `recoveryUntil` for a transaction created before `expiresAt`. No new transaction may be created at or after `expiresAt` for that Quote. The client MUST retain the Quote, Atomic BEEF, remittance, and successful Receipts until it obtains the License or the recovery deadline passes.
...
### 11.5 Deterministic-CBOR HTTP binding
...
| `payment-demand` | signed Payment Demand | `204` | no body |
...
payment-completion = {
  ...
  "quote": signed-object,
  "atomicBeef": bstr,
  "receipts": [1* signed-object]
}
```

The License Request preflight and Quote operations use the Offer payment endpoint. Payment Demand preflight and Payment Delivery use that Demand's endpoint. Payment Completion and License recovery use the Offer payment endpoint. A client MUST dispatch by the signed endpoint on each object; it MUST NOT rewrite all Demand destinations to the issuer origin. An implementation MAY mount all roles at the same URL, but MUST still verify every signed role and Payee identity independently.
...
8. **AI training.** A dataset Offer grants `train` for a stated purpose but not `display`, `unwrap`, or `distribute`. A conforming trainer records `inputTo` provenance where required and rejects an attempted raw export.
9. **Rental.** A License permits `play` until a fixed time and allows 24 hours offline. Playback works offline inside that interval and fails after it. A later status change does not claim to erase a raw CEK previously delivered.
10. **Unknown semantics.** A client that understands the Header but not a critical policy term or mechanism rejects that acquisition profile and may select another fully supported profile.
11. **Tampering.** Modified Header CBOR, wrong ciphertext digest, wrong CEK, reordered records, modified AAD fields, reused payment demands, missing Payee receipts, C2PA ingredient mismatch, authority cycles, and composition cycles are all rejected.
...
An end-to-end reference deployment has five visible role classes: a creator wizard that produces the protected Asset and Offer; a content host implementing the `ContentSink`/`ContentSource` boundary; an issuer endpoint implementing section 11.5; one independently routable delivery service and receiving BRC-100 wallet for every Payee; and a player that separates non-spending preflight from the buyer wallet's `createAction`. The issuer and Payee services need not share an origin, process, operator, or wallet substrate. The Payee wallets, rather than the coordinator, receive the Demand outputs through `internalizeAction`. The issuer persists immutable Quotes and Demands, payment claims and Receipts, issued Licenses, and protected CEKs needed for recovery; the buyer persists its funded fan-out state until completion or `recoveryUntil`.

A single-process reference server MAY combine those roles with an in-memory store. A horizontally scaled or value-bearing service needs a transactional shared Payment Demand ledger, durable License recovery, encrypted CEK storage, independently controlled wallet identities, authenticated creator publication, bounded public endpoints, and CHIRP, UHRP, or HTTPS ciphertext retention. Those operational substitutions do not change the signed LCH objects.
