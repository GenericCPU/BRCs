...
The License Request preflight and Quote operations use the Offer payment endpoint. Payment Demand preflight and Payment Delivery use that Demand's endpoint. Payment Completion and License recovery use the Offer payment endpoint. An implementation MAY mount all roles at the same URL, but MUST still verify every signed role and Payee identity independently.
...
An end-to-end reference deployment has five visible roles: a creator wizard that produces the protected Asset and Offer; a content host implementing the `ContentSink`/`ContentSource` boundary; an issuer endpoint implementing section 11.5; one receiving BRC-100 wallet for every Payee; and a player that separates non-spending preflight from the buyer wallet's `createAction`. The Payee wallets, rather than the coordinator, receive the Demand outputs through `internalizeAction`. The issuer persists immutable Quotes and Demands, payment claims and Receipts, issued Licenses, and protected CEKs needed for recovery.
