# BRC-141: Block Media Format (BMF) — Composable On-Chain Audio/Video
...
2. **Manifest encodings.** A `.bmf` MAY be written in either encoding, and a conformant player SHOULD accept both:
   ...
   **A. Cue form (LRC-compatible).** References components by local filename. Lines are `[mm:ss.cc]<name>`, with optional `# key: value` headers:
   ...
   # tempo: 110
   [00:00.00]countdown-08-sos.webp
   [00:04.36]verse-1-05-power.webp
   [00:08.73]countdown-05-sos.webp
   ```
   ...
   **B. JSON form.** Carries on-chain **txid** references (the pure on-chain form):
   ...
   {
     "bmf": 0,
     "audio": { "tx": "<txid>", "name": "supersonic.flac" },
     "tempo": 110,
     "scenes": [
       { "t": 0.00, "tx": "<txid>", "name": "countdown-08-sos.webp" },
       { "t": 4.36, "tx": "<txid>", "name": "verse-1-05-power.webp" },
       { "t": 8.73, "tx": "<txid>", "name": "countdown-05-sos.webp" }
     ]
   }
   ```
...
4. **First real-world use:** the "Supersonic" music-video mint, whose components (SOS-flicker and tube-power loops, master audio) are individually provenanced and referenced by a single `.bmf`<sup>[2](#footnote-2)</sup>.
...
- <a name="footnote-1">1</a>: Multi-Party Token (MPT) protocol — BRC-113, https://github.com/bitcoin-sv/BRCs
